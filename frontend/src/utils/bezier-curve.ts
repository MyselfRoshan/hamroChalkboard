...
function gradient(a: Point, b: Point): number {
    return (b[1] - a[1]) / (b[0] - a[0])
}
    ...
    for (var i = 1; i < points.length; i++) {
        ...
        if (nexP) {
            ...
            const filteredPoints = points.filter((point, index, array) => {
                // Ensure there is a next point to compare
                if (index === array.length - 1) return false

                const nextPoint = array[index + 1]

                // Get x and y coordinates from the tuples
                const [x, y] = point
                const [nextX, nextY] = nextPoint

                // Check if the difference in x or y coordinates is greater than 2
                const xDifference = Math.abs(nextX - x)
                const yDifference = Math.abs(nextY - y)

                return xDifference > 4 || yDifference > 4
            })
        }
